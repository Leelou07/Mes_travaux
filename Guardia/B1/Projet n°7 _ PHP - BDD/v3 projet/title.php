...
<div class="grid_10">
    <div class="box round first grid">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($title  == NULL) {
                echo "<span style='color:red;font-size:18px;'>ERROR title is not rempli</span>";
            } 
            else {
                if ($logo == NULL) {
                    $majtitle= $db->prepare("UPDATE title SET title = '$title'");
                    $majtitle->execute(array($title));
                    if ($title > 0){
                        echo "<span style='color:red;font-size:18px;'>SUCCES : TITLE IS GOOD</span>";
                    } 
                    else {
                        echo "<span style='color:red;font-size:18px;'>FAILED : TITLE IS NOT GOOD</span>";
                    }
                }   
            }
        }
