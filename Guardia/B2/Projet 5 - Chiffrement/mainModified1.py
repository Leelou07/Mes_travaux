...
def argument():
    ...
    if mode == 0:
        time1 = time.time()
        message = [message[i:i+256] for i in range(0, len(message), 256)] #Fractionne par 256 charactere
        ...
        for i in range(len(messages)):
            ...
            for KEY in keys:
                try:
                    decrypted_message = Decrypt(KEY, messages[i])
                    decrypted_messages.append(decrypted_message.message)
                    decryption_attempted = True
                    print("Clé trouvée: " + KEY)
                    break  # Sortir de la boucle si une clé a réussi
                except Exception as e:
                    continue

            if not decryption_attempted:
                decrypted_messages.append(messages[i])  # Ajouter le message original si le déchiffrement a échoué

        file = open('dechi.txt', 'w', encoding="utf-8")
        file.write(''.join(decrypted_messages))
        print(round(time.time() - time1, 2), 'sec')
